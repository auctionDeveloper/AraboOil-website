...
const BASE = "https://raw.githubusercontent.com/auctionDeveloper/Fueloil-json-data/main/data/ldo/";

const objectiveFiles = {
  Supplier: "LdoSupplier.json",
  Trader: "LdoTrader.json",
  Manufacturer: "LdoManufacture.json",
  Importer: "LdoImporter.json",
  Distributor: "LdoDistributer.json",
};

export default function LDOProductPage({ data }) {
  const [objectiveMap, setObjectiveMap] = useState({});
  const [loading, setLoading] = useState(true);

  const metaDesc = data.description?.substring(0, 160);
  const seoURL = `https://fueloil.in/product/ldo/${data.slug}`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <Helmet>
        ...
        <meta name="keywords" content={`${data.productname}, ${data.city}, LDO supplier, Light Diesel Oil, FuelOil.in`} />
