...
// ✅ Import all 5 Solvent JSON files
import supplierData from '../data/solvent/SolventSupplier.json';
import traderData from '../data/solvent/SolventTrader.json';
import manufacturerData from '../data/solvent/SolventManufacture.json';
import importerData from '../data/solvent/SolventImporter.json';
import distributerData from '../data/solvent/SolventDistributer.json';

export default function SolventProductPage({ data }) {
  const metaDesc = data.description?.substring(0, 160);
  const seoURL = `https://fueloil.in/product/solvent/${data.slug}`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <Helmet>
        ...
        <meta name="keywords" content={`${data.productname}, ${data.city}, Solvent supplier, FuelOil.in`} />
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-8">
        {data.images.map((img, idx) => (
          <img key={idx} src={img} alt="Solvent" className="rounded-md object-cover h-32 w-full" />
        ))}
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-8 mb-12">
        ...
        <div className="md:w-1/2 flex justify-center">
          <img src={data.image1} alt="Main Solvent" className="rounded-md shadow-md max-w-[300px]" />
        </div>
      </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-blue-700 text-sm text-center">
              {list.map((item) => (
                <a
                  ...
                  href={`/solvent/${item.subproduct.toLowerCase().replace(/\s+/g, '-')}/${objective.toLowerCase()}/${item.city.toLowerCase().replace(/\s+/g, '-')}`}
