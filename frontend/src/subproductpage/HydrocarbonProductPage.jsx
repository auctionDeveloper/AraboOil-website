...
// ✅ Import all 5 Hydrocarbon JSON files
import supplierData from '../data/hydrocarbon/HydroCarbonSupplier.json';
import traderData from '../data/hydrocarbon/HydroCarbonTrader.json';
import manufacturerData from '../data/hydrocarbon/HydroCarbonManufacture.json';
import importerData from '../data/hydrocarbon/HydroCarbonImporter.json';
import distributerData from '../data/hydrocarbon/HydrocarbonDistributer.json';

export default function HydrocarbonProductPage({ data }) {
  const metaDesc = data.description?.substring(0, 160);
  const seoURL = `https://fueloil.in/product/hydrocarbon/${data.slug}`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <Helmet>
        ...
        <meta name="keywords" content={`${data.productname}, ${data.city}, Hydrocarbon supplier, FuelOil.in`} />
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-8">
        {data.images.map((img, idx) => (
          <img key={idx} src={img} alt="Hydrocarbon" className="rounded-md object-cover h-32 w-full" />
        ))}
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-8 mb-12">
        ...
        <div className="md:w-1/2 flex justify-center">
          <img src={data.image1} alt="Main Hydrocarbon" className="rounded-md shadow-md max-w-[300px]" />
        </div>
      </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-blue-700 text-sm text-center">
              {list.map((item) => (
                <a
                  ...
                  href={`/hydrocarbon/${item.subproduct.toLowerCase().replace(/\s+/g, '-')}/${objective.toLowerCase()}/${item.city.toLowerCase().replace(/\s+/g, '-')}`}
