...
// ✅ Import all 5 Lamp Oil JSON files
import supplierData from '../data/lampoil/LampoilSupplier.json';
import traderData from '../data/lampoil/LampoilTrader.json';
import manufacturerData from '../data/lampoil/LampoilManufacture.json';
import importerData from '../data/lampoil/LampoilImporter.json';
import distributerData from '../data/lampoil/LampoilDistributer.json';

export default function LampOilProductPage({ data }) {
  const metaDesc = data.description?.substring(0, 160);
  const seoURL = `https://fueloil.in/product/lamp-oil/${data.slug}`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <Helmet>
        ...
        <meta name="keywords" content={`${data.productname}, ${data.city}, Lamp Oil supplier, FuelOil.in`} />
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-8">
        {data.images.map((img, idx) => (
          <img key={idx} src={img} alt="Lamp Oil" className="rounded-md object-cover h-32 w-full" />
        ))}
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-8 mb-12">
        ...
        <div className="md:w-1/2 flex justify-center">
          <img src={data.image1} alt="Main Lamp Oil" className="rounded-md shadow-md max-w-[300px]" />
        </div>
      </div>
      ...
      <div className="mt-12 border-t pt-6">
        <h2 className="text-2xl font-bold mb-4 text-center text-[#980000]">Explore Lamp Oil by Objective</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-blue-700 text-sm text-center">
              {list.map((item) => (
                <a
                  ...
                  href={`/lamp-oil/${item.subproduct.toLowerCase().replace(/\s+/g, '-')}/${objective.toLowerCase()}/${item.city.toLowerCase().replace(/\s+/g, '-')}`}
