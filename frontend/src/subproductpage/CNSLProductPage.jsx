...
// ✅ Import all 5 CNSL Resin JSON files
import supplierData from '../data/cnslresin/CnslSupplier.json';
import traderData from '../data/cnslresin/CnslTrader.json';
import manufacturerData from '../data/cnslresin/CnslManufacture.json';
import importerData from '../data/cnslresin/CnslImporter.json';
import distributerData from '../data/cnslresin/CnslDistributer.json';

export default function CnslResinProductPage({ data }) {
  const metaDesc = data.description?.substring(0, 160);
  const seoURL = `https://fueloil.in/product/cnsl-resin/${data.slug}`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <Helmet>
        ...
        <meta name="keywords" content={`${data.productname}, ${data.city}, CNSL Resin supplier, FuelOil.in`} />
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-8">
        {data.images.map((img, idx) => (
          <img key={idx} src={img} alt="CNSL Resin" className="rounded-md object-cover h-32 w-full" />
        ))}
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-8 mb-12">
        ...
        <div className="md:w-1/2 flex justify-center">
          <img src={data.image1} alt="Main CNSL Resin" className="rounded-md shadow-md max-w-[300px]" />
        </div>
      </div>
      ...
      <div className="mt-12 border-t pt-6">
        <h2 className="text-2xl font-bold mb-4 text-center text-[#980000]">Explore CNSL Resin by Objective</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-blue-700 text-sm text-center">
              {list.map((item) => (
                <a
                  ...
                  href={`/cnsl-resin/${item.subproduct.toLowerCase().replace(/\s+/g, '-')}/${objective.toLowerCase()}/${item.city.toLowerCase().replace(/\s+/g, '-')}`}
