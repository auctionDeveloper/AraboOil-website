...
export default function CnslResinObjectivesPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const keys = ['supplier', 'trader', 'manufacturer', 'importer', 'distributer'];

        const results = await Promise.all(
          files.map(file =>
            fetch(`${jsonUrls.data.cnslresin[file]}?nocache=${Date.now()}`).then(res => res.json())
          )
        );
        ...
        console.error('Failed to load CNSL Resin data:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="text-center py-10">Loading CNSL Resin data...</div>;
  ...
  const seoURL = `https://fueloil.in/cnslresin/${objective}/${city}`;

  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <Helmet>
        <title>CNSL Resin {objective} in {data.city} | FuelOil.in</title>
        <meta name="description" content={metaDesc} />
        <meta name="keywords" content={`${data.productname}, ${data.city}, CNSL Resin ${objective}, FuelOil.in`} />
