...
// ✅ Import Yellow Base Oil objective data
import supplierData from '../data/yellowbaseoil/YboSupplier.json';
import traderData from '../data/yellowbaseoil/YboTrader.json';
import manufacturerData from '../data/yellowbaseoil/YboManufacture.json';
...
export default function YellowBaseOilObjectivesPage() {
  ...
  const seoURL = `https://fueloil.in/yellow-base-oil/${subproduct}/${objective}/${city}`;
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-blue-700 text-sm text-center">
              {list.map((item) => (
                <a
                  ...
                  href={`/yellow-base-oil/${item.subproduct.toLowerCase().replace(/\s+/g, '-')}/${objective.toLowerCase()}/${item.city.toLowerCase().replace(/\s+/g, '-')}`}
