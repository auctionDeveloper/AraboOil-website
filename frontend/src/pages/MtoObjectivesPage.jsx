...
export default function MtoObjectivesPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const objectives = ['MtoSupplier', 'MtoTrader', 'MtoManufacture', 'MtoImporter', 'MtoDistributer'];
        const keys = ['supplier', 'trader', 'manufacturer', 'importer', 'distributor'];

        const results = await Promise.all(
          objectives.map(file =>
            fetch(`${jsonUrls.data.mto[file]}?nocache=${Date.now()}`).then(res => res.json())
          )
        );
        ...
        console.error("Failed to load MTO data:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="text-center py-10">Loading MTO data...</div>;
  ...
  const seoURL = `https://fueloil.in/mto/${objective}/${city}`;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 md:px-8 py-10">
      <Helmet>
        ...
        <meta name="keywords" content={`${data.productname}, ${data.city}, MTO ${objective}, Arabo Oil`} />
