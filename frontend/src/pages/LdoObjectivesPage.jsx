...
export default function LdoObjectivesPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const files = ['LdoSupplier', 'LdoTrader', 'LdoManufacture', 'LdoImporter', 'LdoDistributer'];
        const keys = ['supplier', 'trader', 'manufacturer', 'importer', 'distributor'];

        const results = await Promise.all(
          files.map(file => fetch(jsonUrls.data.ldo[file]).then(res => res.json()))
        );
        ...
        console.error("Failed to load LDO data:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="text-center py-10">Loading LDO data...</div>;
  ...
  const seoURL = `https://fueloil.in/ldo/${objective}/${city}`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <Helmet>
        ...
        <meta name="keywords" content={`${data.productname}, ${data.city}, LDO ${objective}, FuelOil.in`} />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-blue-700 text-sm text-center">
              {list.map((item) => (
                <a
                  ...
                  href={`/ldo/${objectiveKey.toLowerCase()}/${item.city.toLowerCase().replace(/\s+/g, '-')}`}
                  className="hover:text-red-600 underline transition-colors"
                >
                  LDO {objectiveKey} in {item.city}
                </a>
              ))}
            </div>
          </div>
