...
// ✅ Import White Base Oil objective data
import supplierData from '../data/whitebaseoil/WboSupplier.json';
import traderData from '../data/whitebaseoil/WboTrader.json';
import manufacturerData from '../data/whitebaseoil/WboManufacture.json';
import importerData from '../data/whitebaseoil/WboImporter.json';
import distributerData from '../data/whitebaseoil/WboDistributer.json';
...
export default function WhiteBaseOilObjectivesPage() {
  ...
  const seoURL = `https://fueloil.in/white-base-oil/${subproduct}/${objective}/${city}`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <Helmet>
        ...
        <meta name="keywords" content={`${data.productname}, ${data.city}, White Base Oil ${objective}, Arabo Oil`} />
      ...
      <div className="flex flex-col md:flex-row gap-8 mb-12">
        ...
        <div className="md:w-1/2 flex justify-center">
          <img src={data.image1} alt="Main WBO" className="rounded-md w-[250px] shadow-md object-contain" />
        </div>
      </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-blue-700 text-sm text-center">
              {list.map((item) => (
                <a
                  ...
                  href={`/white-base-oil/${item.subproduct.toLowerCase().replace(/\s+/g, '-')}/${objective.toLowerCase()}/${item.city.toLowerCase().replace(/\s+/g, '-')}`}
