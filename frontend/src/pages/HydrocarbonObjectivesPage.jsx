...
export default function HydrocarbonObjectivesPage() {
    ...
    const loadData = async () => {
      try {
        ...
        const results = await Promise.all(
          files.map(file => fetch(jsonUrls.data.hydrocarbon[file]).then(res => res.json()))
        );
        ...
        console.error("Failed to load Hydrocarbon data:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="text-center py-10">Loading Hydrocarbon data...</div>;
  ...
  const seoURL = `https://fueloil.in/hydrocarbon/${objective}/${city}`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <Helmet>
        ...
        <meta name="keywords" content={`${data.productname}, ${data.city}, Hydrocarbon ${objective}, FuelOil.in`} />
      ...
      <div className="text-center mb-8">
        ...
        <h2 className="text-xl text-red-700 font-semibold mt-1">({data.productname})</h2>
      </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-blue-700 text-sm text-center">
              {list.map((item) => (
                <a
                  ...
                  href={`/hydrocarbon/${objectiveKey.toLowerCase()}/${item.city.toLowerCase().replace(/\s+/g, '-')}`}
                  className="hover:text-red-600 underline transition-colors"
                >
                  Hydrocarbon {objectiveKey} in {item.city}
                </a>
              ))}
            </div>
          </div>
