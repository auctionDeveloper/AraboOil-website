...
export default function SolventObjectivesPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const files = ['SolventSupplier', 'SolventTrader', 'SolventManufacture', 'SolventImporter', 'SolventDistributer'];
        const keys = ['supplier', 'trader', 'manufacturer', 'importer', 'distributor'];

        const results = await Promise.all(
          files.map(file => fetch(jsonUrls.data.solvent[file]).then(res => res.json()))
        );

        const objMap = {};
        keys.forEach((key, idx) => {
          objMap[key] = results[idx];
        });

        setObjectiveMap(objMap);
      } catch (err) {
        console.error('Failed to load Solvent data:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="text-center py-10">Loading Solvent data...</div>;
  ...
  const seoURL = `https://fueloil.in/solvent/${objective}/${city}`;
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-blue-700 text-sm text-center">
              {list.map((item) => (
                <a
                  ...
                  href={`/solvent/${objectiveKey.toLowerCase()}/${item.city.toLowerCase().replace(/\s+/g, '-')}`}
                  className="hover:text-red-600 underline transition-colors"
                >
                  Solvent {objectiveKey} in {item.city}
                </a>
              ))}
            </div>
          </div>
