...
export default function LampOilObjectivesPage() {
    ...
    const loadData = async () => {
      try {
        ...
        const results = await Promise.all(
          objectives.map(file => fetch(jsonUrls.data.lampoil[file]).then(res => res.json()))
        );
        ...
        console.error("Failed to load Lamp Oil data:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="text-center py-10">Loading Lamp Oil data...</div>;
  ...
  const seoURL = `https://fueloil.in/lamp-oil/${objective}/${city}`;

  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <Helmet>
        <title>Lamp Oil {objective} in {data.city} | FuelOil.in</title>
        <meta name="description" content={metaDesc} />
        <meta name="keywords" content={`${data.productname}, ${data.city}, Lamp Oil ${objective}, FuelOil.in`} />
