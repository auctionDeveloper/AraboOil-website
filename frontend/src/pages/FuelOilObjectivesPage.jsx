...
export default function FuelOilObjectivesPage() {
    ...
    const loadData = async () => {
      try {
        ...
        const results = await Promise.all(
          files.map(file => fetch(jsonUrls.data.fueloil[file]).then(res => res.json()))
        );
        ...
        console.error("Failed to load Fuel Oil data:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="text-center py-10">Loading Fuel Oil data...</div>;
  ...
  const seoURL = `https://fueloil.in/fuel-oil/${objective}/${city}`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <Helmet>
        ...
        <meta name="keywords" content={`${data.productname}, ${data.city}, Fuel Oil ${objective}, FuelOil.in`} />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-blue-700 text-sm text-center">
              {list.map((item) => (
                <a
                  ...
                  href={`/fuel-oil/${objectiveKey.toLowerCase()}/${item.city.toLowerCase().replace(/\s+/g, '-')}`}
                  className="hover:text-red-600 underline transition-colors"
                >
                  Fuel Oil {objectiveKey} in {item.city}
                </a>
              ))}
            </div>
          </div>
